...
用户提供的真实响应包含 9 条 Collection、`total=9`，同时仍返回
`nextPath=/api/collections.list?limit=25&offset=25`。原实现按已返回条数校验
下一页偏移，未先识别末页，导致连接测试误报 `outline_scan_incomplete`。

现已允许末页保留 `offset+limit` 的下一页偏移，并在末页结束扫描；仍校验
地址、偏移推进及非末页连续性。新增 42 组分页用例，覆盖三个列表端点、
有无 total、空页、短页、整页边界与多页，并补充末页跨域和跳页拒绝测试。
修复前已用 9 条模拟记录复现错误；修复后 Outline 包 `go test -race` 通过。

原测试遗漏了这种末页仍带 nextPath 的响应。本次没有使用用户 API Key
请求真实实例，也未执行生产部署或浏览器端到端验证；需重新构建部署后端后验收。
测试数据不包含用户凭据或真实文档内容。
